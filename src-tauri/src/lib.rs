...
pub fn run() {
    ...
    let result = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let (saved_position, saved_size, saved_settings_size) =
                if let Ok(dir) = handle.path().app_data_dir() {
                    ...
                    (cfg.overlay_position, saved_size, cfg.settings_size)
                } else {
                    (None, None, None)
                };
            ...
            // Pre-apply the persisted settings size and re-center, so the
            // first `open_settings` paints at the user's last size instead
            // of the conf default. Stays hidden until requested.
